...
if (_respawn && _respawnmax > 0 &&  !_surrended  && _allow_respawn) then {
    ...
    if(_is_heli_group) then {
        _leader setVehicleInit "null = [this] execVM 'germandayz\ai\SARGE\SAR_trace_from_vehicle.sqf';this setIdentity 'id_SAR_sold_lead';";    
    } else {
        _leader setVehicleInit "null = [this] execVM 'germandayz\ai\SARGE\SAR_trace_entities.sqf';this setIdentity 'id_SAR_sold_lead';";
    ...
    _leader addMPEventHandler ["MPkilled", {Null = _this execVM "germandayz\ai\SARGE\SAR_aikilled.sqf";}]; 
    _leader addMPEventHandler ["MPHit", {Null = _this execVM "germandayz\ai\SARGE\SAR_aihit.sqf";}];

    _cond="(side _this == west) && (side _target == SAR_AI_friendly_side) && ('ItemBloodbag' in magazines _this)";

    [nil,_leader,rADDACTION,"Give me a blood transfusion!", "germandayz\ai\SARGE\SAR_interact.sqf","",1,true,true,"",_cond] call RE;
    ...
    _i=0;
    {
        ...
        if (_i>1) then {
            ...
            _newunit setVehicleInit "null = [this] execVM 'germandayz\ai\SARGE\SAR_trace_entities.sqf';this setIdentity 'id_SAR';";
            
            if(_is_heli_group) then {
                _newunit setVehicleInit "null = [this] execVM 'germandayz\ai\SARGE\SAR_trace_from_vehicle.sqf';this setIdentity 'id_SAR';";    
            } else {
                _newunit setVehicleInit "null = [this] execVM 'germandayz\ai\SARGE\SAR_trace_entities.sqf';this setIdentity 'id_SAR';";
            };

            _newunit addMPEventHandler ["MPkilled", {Null = _this execVM "germandayz\ai\SARGE\SAR_aikilled.sqf";}]; 
            _newunit addMPEventHandler ["MPHit", {Null = _this execVM "germandayz\ai\SARGE\SAR_aihit.sqf";}]; 
    ...
    {				
        ...
        //_heli addEventHandler ["HandleDamage", {returnvalue = _this execVM "germandayz\ai\SARGE\SAR_ai_vehicle_hit.sqf";}];  
        _heli addMPEventHandler ["MPHit", {Null = _this execVM "germandayz\ai\SARGE\SAR_aihit.sqf";}];  
