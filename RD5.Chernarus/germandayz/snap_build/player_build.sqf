...
_distance = 100; //radius for building objects
_needText = localize "str_epoch_player_246";

if(_isPole) then {
	_distance = 200; //radius to prevent building around a plotpole
};
...
if (_hasrequireditem) then {
	...
	if(!_cancel) then {
		...
		if (_proceed) then {
			...
			if (_classname in ["Plastic_Pole_EP1_DZ","Land_HBarrier5_DZ","Land_HBarrier3_DZ","Land_HBarrier1_DZ","MetalFloor_DZ","CinderWallDoorSmallLocked_DZ","CinderWallDoorLocked_DZ","Cinderwall_DZ"]) then {
			...
			if(_num_removed == 1) then {
				...
				if(_lockable > 1) then {
					...
					if(_object isKindOf "Land_Fire_DZ") then {
						...
						PVDZE_obj_Publish = [dayz_playerUID,_object,[_dir,_location],_classname];
						publicVariableServer "PVDZE_obj_Publish";
						...
						if (_classname in ["Plastic_Pole_EP1_DZ"]) then {
							...
							cutText [format["The PlotPole Code is: ID %1 I UID %2 I pUID %3 - Use the Action Menu on PlotPole AFTER a server restart to get the code.",_PoleID,_PoleUID,_pUID], "PLAIN DOWN", 5];
						};
					};
				};
			} else {
